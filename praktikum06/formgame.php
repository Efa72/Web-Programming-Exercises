...
<body>
	<h1>Test Login Game Tebak Angka</h1>
	<?php
	...
	setcookie("visits", 1, time()+3*30*24*3600,"/");
	// mengeset cookie kunjungan terakhir
	setcookie("lastvisit", date("d-m-Y H:i:s"), time()+3*30*24*3600,"/");
	// setelah mengeset cookie awal di atas, redirect ke halaman depan
	header("Location:formprojectlatihan.php");
}
...
if (isset($_COOKIE['username'])){
	...
	setcookie("lastvisit", date("d-m-Y H:i:s"), time()+3*30*24*3600,"/");
	echo "<p>Ayo main <a href='game.php'>game</a> ...</p>";
}
 else {
...
?>
	...
	<form method="post" action="formprojectlatihan.php">
